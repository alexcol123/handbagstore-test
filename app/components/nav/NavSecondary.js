...
const NavSecondary = ({ categoriesLinks}) => {
  ...
  return (
    <div>
      <div className="flex md:hidden items-center justify-center p-1">      <ThemeToggle />
      </div>
      <ul className='hidden md:flex items-center justify-center gap-2 md:gap-5 p-1 text-sm  uppercase '>
        {categoriesLinks.map((item) => {
          return <Link key={item.name} href={item.href} className=" py-1 pl-3 hover:text-primary hover:bg-base-100" >
            {item.name}
          </Link>
        })}
